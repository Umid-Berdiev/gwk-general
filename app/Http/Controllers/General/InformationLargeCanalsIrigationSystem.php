...
use App\Models\General\InformationLargeCanalsIrigationSystem as ModelName;
...
class InformationLargeCanalsIrigationSystem extends Controller
{
  ...
  { 
    ...
    // if (auth()->user()->org_name == 'gidromet' || auth()->user()->org_name == 'other') {
      ...
      if ($information_large_canals == 0) {
        ...
        $information_canal->years = $selected_year;
        ...
        $information_large_canals = ModelName::where('years', $selected_year)->orderby('id', 'ASC')->get();
  ...
  public function update(Request $request)
  {
    switch ($request->func) {
      case "canal_bandwidth":
        ModelName::where('id', $request->id)->update(['user_id' => auth()->id(), 'is_approve' => false, 'canal_bandwidth' => $request->param]);
        break;
      case 'average_water':
        ModelName::where('id', $request->id)->update(['user_id' => auth()->id(), 'is_approve' => false, 'average_water' => $request->param]);
        break;
      case 'canal_main_structures':
        ModelName::where('id', $request->id)->update(['user_id' => auth()->id(), 'is_approve' => false, 'canal_main_structures' => $request->param]);
        break;
    }
  }
  ...
  public function accept(Request $request)
  {
    if (auth()->user()->org_name == 'gidromet') {
      ModelName::where('years', $request->get('year'))->update(['user_id' => auth()->id(), 'is_approve' => true]);
    }
