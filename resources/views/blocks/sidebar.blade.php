  ...
  <div class="sidebar-sticky">
    @if(!starts_with(Route::current()->uri, 'admin'))
      <nav class="navbar flex-column p-0">
        ...
        <a class="nav-link {{ starts_with(Route::current()->uri, 'report') ? 'active' : '' }}"
           href="{{route('report.index')}}" @click="persist('anotherOne')">
          {{ __("messages.Reports") }}
        </a>

        <a class="nav-link" type="button" :class="{'active' : isShown5}" @click="persist('isShown5')">
          {{ __("messages.References") }}
          <i class="float-right bi" :class="{ 'bi-chevron-down' : !isShown5, 'bi-chevron-up' : isShown5 }"></i>
        </a>
        <div v-show="isShown5" class="navbar-collapse">
          <nav class="navbar-nav flex-column pl-4">
            <a href="{{route('units-index')}}"
               class="nav-link {{ url()->current() == route('units-index') ? 'active' : '' }}">
              {{ __("messages.Units") }}
            </a>
          </nav>
        </div>
